...
import { withoutPreviewBase, withPreviewBase } from "./url";
...
function applyCoverPosition(settings: PreviewSettingsV1) {
  for (const article of document.querySelectorAll<HTMLElement>("article[data-pagefind-body]")) {
    ...
    cover.classList.toggle(
      "cover--down",
      settings.variant === "motion-primitives" && settings.coverPosition === "down",
    );
    cover.classList.toggle(
      "article-cover",
      settings.variant === "cult-ui" && settings.coverPosition === "up",
    );
    cover.classList.toggle(
      "article-cover-down",
      settings.variant === "cult-ui" && settings.coverPosition === "down",
    );
    cover.classList.toggle(
      "cover--head",
      settings.variant === "aceternity" && settings.coverPosition === "up",
    );
    cover.classList.toggle(
      "cover--body",
      settings.variant === "aceternity" && settings.coverPosition === "down",
    );
    cover.classList.toggle(
      "post-cover-down",
      settings.variant === "organique" && settings.coverPosition === "down",
    );
...
async function onMessage(event: MessageEvent) {
  ...
  if (
    ...
    /^\/(?!\/)/.test(event.data.path) &&
    !event.data.path.includes("..")
  ) {
    try {
