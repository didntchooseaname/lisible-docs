/**
 * Browser-facing stylesheet for the RSS feeds.
 *
 * Locale handling lives inside the XSL (it branches on `channel/language`), so a
 * single stylesheet serves both feeds and every variant.
 */
export function rssStylesheet(title: string, accent: string): string {
