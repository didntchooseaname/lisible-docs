...
Ces trois schémas sont des **SVG écrits en ligne dans le HTML**, pas des fichiers image chargés par `<img>`. C'est ce qui change tout: un SVG inline hérite des variables CSS de la page, donc ses traits suivent l'accent choisi dans l'en-tête et ses surfaces suivent le thème actif, en direct, sans JavaScript et sans requête supplémentaire. Un SVG servi via `<img>` reste isolé de la page et ne peut pas faire ça.

<Diagram id="islands" locale="fr" />

<Diagram id="pipeline" locale="fr" />

<Diagram id="tokens" locale="fr" />
