...
Lisible uses Astro’s native i18n routing: French documentation routes have no prefix and English routes live under `/en/`.[^astro-i18n] The canonical landing pages live at `/fr/` and `/en/`. The `x-default` `/` entry is the exception: its default HTML is English.
