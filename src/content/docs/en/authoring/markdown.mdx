...
Dimensions are not your responsibility: at build time Lisible reads the intrinsic size of every local image (SVG, PNG, JPEG, GIF) and sets `width`, `height`, `loading="lazy"` and `decoding="async"` on the emitted tag. Space is reserved before the image loads, so it never shifts the layout.
