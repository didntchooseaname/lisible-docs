...
Files under `shared/` take no runtime dependency beyond those installed at the repository root: `shared/` has no `node_modules`, so resolution walks up to the root.
